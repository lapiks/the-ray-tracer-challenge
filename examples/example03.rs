...
use ray_tracer::{Camera, World, Object, shapes::{Sphere, Shape, Plane}, Color, Material, PointLight, transformations, Pattern, pattern::{PlainPattern, PatternObject, StrippedPattern, GradientPattern}};
...
fn main() {
        ...
        .with_material(
            Material::default()
                .with_pattern(
                    ...
                    .with_transform(
                        &(DMat4::from_rotation_z(PI / 4.0) * DMat4::from_translation(dvec3(-1.0, 0.0, 0.0)) * DMat4::from_scale(dvec3(2.0, 2.0, 2.0)))
                    )
                )
                .with_diffuse(0.7)
                .with_specular(0.3)
    ...
    let camera = Camera::new(1000, 1000, PI / 3.0)
        .with_transform(
